//Logan Crawford & Jake Nguyen - OS Homework
...
//hardcoded sudoku puzzle that will be checked for validation
int sudoku[9][9] = {
	{6, 2, 4, 	5, 3, 9,	 1, 8, 7},
	(5, 1, 9, 	7, 2, 8, 	 6, 3, 4),
	(8, 3, 7, 	6, 1, 4, 	 2, 9, 5),
...
//initalize parameters and method for validating a column
...
//Initalization and method for validating a row
void *isRowValid(void* param) {
	paramters *params = {parameters*)param;
	int row = params->row;
	int col = params->column;
	if(col != 0 || row > 8) {
...
//Initalization and method for validating a 3x3 grid
...
//main method that will handle threads and checking the sudoku puzzle
int main() {
	...
	for(i = 0; i < 9; i++) {
		for(j = 0; j < 9; j++) {
			...
			if(i == 0) {
				...
				columnData->row = i;
				columnData->column = j;
				pthreaed_create(&threads[threadIndex++], NULL, isColumnValid, columnData);
			}
			if(j == 0) {
	...
	//Return if sudoku puzzle is valid or invalid
